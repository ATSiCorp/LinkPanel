...
class AuthController extends Controller
{
    ...
    public function appLogin(Request $request)
    {
        ...
        if (!$user) {
            return response()->json([
                'message' => __('linkpanel.invalid_login_message'),
                'errors' => __('linkpanel.invalid_login')
            ], 401);
        }
    ...
    */
    public function login(Request $request)
    {
        ...
        $user->jwt = JWT::encode(['iat' => time(),'exp' => time() + config('linkpanel.jwt_refresh')], config('linkpanel.jwt_secret').'-Rfs');
        $user->save();

        return response()->json([
            'access_token' => JWT::encode(['iat' => time(),'exp' => time() + config('linkpanel.jwt_access')], config('linkpanel.jwt_secret').'-Acs'),
    ...
    */
    public function refresh(Request $request)
    {
        ...
        if ($user) {
            ...
            return response()->json([
                'message' => __('linkpanel.invalid_token_message'),
                'errors' => __('linkpanel.invalid_token')
            ], 401);
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->newusername) {
            ...
            if (!Auth::where('username', $newuser)->first()) {
                ...
                return response()->json([
                    'message' => __('linkpanel.username_conflict_message'),
                    'errors' => __('linkpanel.username_conflict')
                ], 409);
            }
        }
