...
class LinkPanelUpdate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'linkpanel:update';
     ...
     * @var string
     */
    protected $description = 'Update LinkPanel';
    ...
    public function handle()
    {
        ...
        foreach ($servers as $server) {
            ...
            $ssh->login('linkpanel', $server->password);
            $ssh->setTimeout(360);
        ...
        $ssh->login('linkpanel', $server->password);
        $ssh->setTimeout(360);
        $ssh->exec('echo '.$server->password.' | sudo -s cd /var/www/html/utility/linkpanel-update && sh run.sh');
        $ssh->exec('exit');
