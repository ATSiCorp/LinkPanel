...
if [ "$ID" = "ubuntu" ]; then
    case $VERSION in
        ...
        *)
            ...
            echo "LinkPanel requires Linux Ubuntu 20.04 LTS"
            echo "${reset}"
            exit 1;
            break
            ;;
...
if [ "$(id -u)" = "0" ]; then
    ...
    clear
    echo "${bgred}${white}${bold}"
    echo "You have to run LinkPanel as root. (In AWS use 'sudo -s')"
    echo "${reset}"
    exit 1
fi
...
LINKPANELBULD=/var/www/html/build_$SERVERID.php
sudo touch $LINKPANELBULD
sudo cat > "$LINKPANELBULD" <<EOF
$BUILD
EOF
LINKPANELPING=/var/www/html/ping_$SERVERID.php
...
sudo touch $ARTISAN
UPDATE=/var/www/html/utility/linkpanel-update/run.sh
sudo touch $UPDATE
...
echo "***********************************************************"
echo ""
echo " SSH root user: linkpanel"
echo " SSH root pass: $PASS"
echo " MySQL root user: linkpanel"
